...
def donut_svg(pct_val, color, size=72):
    r = 26; cx = cy = size // 2; circ = 2 * math.pi * r
    arc = (pct_val / 100) * circ
    offset = circ / 4
    return (
        f'<svg width="{size}" height="{size}" viewBox="0 0 {size} {size}">'
        f'<circle cx="{cx}" cy="{cy}" r="26" fill="none" stroke="#b0bfc9" stroke-width="9"/>'
        f'<circle cx="{cx}" cy="{cy}" r="26" fill="none" stroke="{color}" stroke-width="9"'
        f' stroke-dasharray="{arc:.1f} {circ-arc:.1f}"'
        f' stroke-dashoffset="{offset:.1f}" stroke-linecap="round"/>'
        f'<text x="{cx}" y="{cy-3}" text-anchor="middle" font-size="11" font-weight="700" fill="{color}">{pct_val:.0f}%</text>'
        f'<text x="{cx}" y="{cy+8}" text-anchor="middle" font-size="7" fill="#2f3e4d">コア</text>'
        f'</svg>'
    )
